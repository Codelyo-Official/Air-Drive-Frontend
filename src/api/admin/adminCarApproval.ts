  ...
  const useAdminCarsList = (filters?: {
    ...
    return useQuery<AdminCar[], Error>({
      ...
      queryFn: async () => {
        ...
        if (filters?.owner_id)
          url.searchParams.append("owner_id", String(filters.owner_id));
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.detail ||
              `Failed to fetch cars. Status: ${response.status}`
          );
        }
  ...
  const updateCar = useMutation<UpdateCarResponse, Error, UpdateCarPayload>({
    mutationFn: async ({ car_id, updates }) => {
      ...
      const cleanUpdates = Object.entries(updates).reduce(
        (acc, [key, value]) => {
          if (value !== undefined && value !== null && value !== "") {
            ...
            if (
              ["year", "seats", "latitude", "longitude"].includes(key) &&
              typeof value === "string"
            ) {
      ...
      console.log(
        "Updating car:",
        car_id,
        "with cleaned updates:",
        cleanUpdates
      );

      const response = await fetch(
        `${API_BASE_URL}/api/admin/cars/${car_id}/`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        ...
        if (response.status === 400) {
          ...
          throw new Error(
            errorData.detail ||
              `Failed to update car. Status: ${response.status}`
          );
        }
      }
    ...
    mutationFn: async (car_id) => {
      ...
      if (!response.ok) {
        ...
        if (response.status === 405) {
          ...
          throw new Error(
            errorData.detail ||
              `Failed to delete car. Status: ${response.status}`
          );
        }
      }
