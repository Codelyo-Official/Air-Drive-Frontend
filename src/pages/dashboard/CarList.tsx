...
const CarList: React.FC = () => {
  ...
  const [view, setView] = useState<"available" | "owner">("owner")
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid")

  const { useOwnerCars, useAvailableCars } = useCar()
  const { data: ownerCars, isLoading: isLoadingOwner, error: errorOwner } = useOwnerCars()
  const { data: availableCars, isLoading: isLoadingAvailable, error: errorAvailable } = useAvailableCars()

  // Choose which cars to show
  const cars: CarWithExtras[] | undefined = view === "owner" ? ownerCars : availableCars
  ...
  // Loading State
  if (view === "owner" ? isLoadingOwner : isLoadingAvailable) {
    return (
  ...
  // Error State
  if (view === "owner" ? errorOwner : errorAvailable) {
    return (
      <div className="min-h-screen bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center py-20">
        ...
        <div className="bg-white/90 backdrop-blur-sm p-4 sm:p-6 rounded-xl shadow-lg border-0">
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex bg-amber-100 rounded-lg p-1">
              <button
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 ${view === "owner"
              ...
              <button
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${view === "available"
                    ? "bg-amber-500 text-white shadow-sm"
                    : "text-amber-700 hover:text-amber-800 hover:bg-amber-50"
                  }`}
            ...
            <div className="flex gap-3">
              ...
              <div className="flex bg-amber-100 rounded-lg p-1">
                <button
                  ...
                  className={`p-2 rounded transition-all duration-200 ${viewMode === "grid" ? "bg-amber-500 " : "hover:bg-amber-50"
                    }`}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                ...
                <button
                  ...
                  className={`p-2 rounded transition-all duration-200 ${viewMode === "list" ? "bg-amber-500" : "hover:bg-amber-50"
                    }`}
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
        ...
        {viewMode === "grid" ? (
          ...
          <div className="space-y-4">
            {filteredCars.map((car) => (
              <div
                key={car.id}
                className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 p-6"
              >
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <div className="text-2xl font-bold text-amber-600">${car.daily_rate}</div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="px-4 py-2 border border-amber-200 text-amber-700 rounded-lg hover:bg-amber-50 transition-colors flex items-center gap-2">
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
